...
{
    char buffer[MAX_STRING_LENGTH]; // jjw patch from 4.2 mem err
    va_list val;
    va_start(val, format);
...
char test_abort(char selectbefore, int file, int sock)
{
    ...
    fd_set rfds;
    struct timeval tv;
    char *result; // jjw patch from 4.2 mem err
    if (selectbefore) {
        tv.tv_sec = 0;
        tv.tv_usec = 0;
        FD_ZERO(&rfds);
    ...
    result = fgets(str, sizeof(str), stdin); // jjw patch from 4.2 mem err
    if ( (result) &&  (strstr(str, "ABOR")) ) { // jjw patch from 4.2 mem err
...
void do_stor(char *filename, int flags)
{
    ...
    if ( attempt_gzip )
    {
        ...
        {
            free(mapped);
            mapped = NULL; // jjw patch from 4.2 mem err
        }
        if (! my_zip_file)
        {
    ...
    if (dataconn())
    {
        ...
        if (post_write_script)
            run_script(post_write_script, mapped);
        if (mapped) // jjw patch from 4.2 mem err
            free(mapped);
        return;
    }
...
void command_retr(char *filename)
{
    ...
    char *mapped=NULL; // jjw patch from 4.2 mem err
    char *buffer;
    int xfer_delay;
    struct timeval wait_time;
        ssize_t send_status;
...
void do_dirlist(char *dirname, char verbose)
{
    ...
    if (dirname[0] == '\0')
        ...
        bftpd_log("Access to  '%s' !\n", mapped);
        if (strncmp(mapped, "/tmp/media/sd", 13) != 0 )
        {
